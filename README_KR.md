...
### 결과 분석
- **Val (평가가치)**: 모델에서 계산한 공정가치로 실제 행사가가 3.5%, 현재 시장의 고정금리가 1.25%인 Callable swap의 가치
#### HW와 LMM의 비교
1. **Val**: 현재 시장의 고정금리인 1.25%를 기준으로 한국과 미국 시장 모두 HW는 고평가를, LMM은 저평가를 하고 있음.
...
4. **Vega**: HW 모델은 한국과 미국시장 모두 1.1%를 넘는 수치를 기록하였고, LMM은 0.11 수준으로 낮게 계산되었으며 이는 LMM 모델이 테너별
...
2. 현재 시장의 고정금리를 기준으로 HW 모델은 채권 가격을 고평가, LMM은 저평가 하는 것이 확인되었음
3. LMM은 테너별 변동성을 사전에 반영하여 HW에 비하여 낮은 베가값을 갖으며 변동성 대비 낮은 변화를 보임
4. HR로 미국 시장과 한국 시장의 유동성과 효율성 차이와 모델의 변동성 민감도를 전부 확인 할 수 있음
5. LMM 에서 베타를 1.5로 사용하고 있으며 이는 일반적인 시장보다 높게 설정되어 있으며 이를 조정시 LMM 결과가 현재 시장의 고정금리에 더 근접할 것으로 예상됨
